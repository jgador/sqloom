...
async function readCliVersion() {
  ...
  if (!versionMatch || !versionMatch[1].trim()) {
    throw new Error(
      `${cliVersionPath} must define <Version> for Sqloom packages.`,
    );
  }
...
function updateVersion(container, version, label) {
  ...
  if (checkOnly) {
    throw new Error(
      `${label} version ${currentVersion ?? "<missing>"} does not match ${version}.`,
    );
  }
...
async function main() {
  ...
  changed =
    updateVersion(
      extensionPackage,
      version,
      "extensions/sqloom/package.json",
    ) || changed;
  ...
  changed =
    updateVersion(
      packageLock.packages?.["extensions/sqloom"],
      version,
      "package-lock extensions/sqloom",
    ) || changed;

  if (!changed) {
    console.log(
      `VS Code extension version is already in sync with CLI version ${version}.`,
    );
    return;
  }
  ...
  console.log(
    `Synced workspace and VS Code extension versions to CLI version ${version}.`,
  );
}
