...
/// </summary>
internal static class TestAppReplaySqlServerDacpacPathResolver
{
    public static string ResolveRequiredPath(ReplayLaunchOptions? launchOptions)
    {
        var dacpacPath = launchOptions?.SqlServerDacpacPath;
        if (string.IsNullOrWhiteSpace(dacpacPath))
        {
            throw new ArgumentException(
                "Sqloom Test App replay requires a SQL Server DACPAC path when SQL Server bootstrap is requested. Pass --sqlserver-dacpac-file <path> or set EndpointReplayRunnerOptions.ReplayLaunchOptions.SqlServerDacpacPath when calling the replay runner directly.");
        }
