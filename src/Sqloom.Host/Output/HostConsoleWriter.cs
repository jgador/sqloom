...
internal sealed class HostConsoleWriter
{
    ...
    public void PrintUsage()
    {
        ...
        Console.WriteLine("    Tune uses --read-only-connection-string when supplied, otherwise it uses the harness session connection string. Advice uses --sqlserver-schema-file when supplied, otherwise it extracts schema SQL from --sqlserver-dacpac-file, the harness manifest DACPAC, or a DACPAC exported from the read-only connection.");
