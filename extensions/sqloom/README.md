...
- Enter a required dashboard-only masked read-only SQL Server connection string.
...
This preview keeps the CLI as the complete workflow engine. The dashboard loads endpoint choices from the selected CLI and harness, keeps the choice only for the open dashboard session, and clears it when that context changes. Dashboard-entered API keys and read-only SQL Server connection strings are masked, are not stored in VS Code settings, and are passed only to the current CLI run. When `OPENAI_API_KEY` is available to VS Code, the dashboard prefills the masked API key field from it, and dashboard and Command Palette tune runs use the key without storing it.
